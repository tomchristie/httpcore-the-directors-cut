...
class TrioStream(AsyncNetworkStream):
    ...
    async def start_tls(self, ssl_context: ssl.SSLContext) -> AsyncNetworkStream:
        trio_ssl_stream = trio.SSLStream(self._stream, ssl_context)
...
class TrioBackend(AsyncNetworkBackend):
    ...
    async def connect(self, origin: Origin) -> AsyncNetworkStream:
        ...
        if origin.scheme == b"https":
            stream = await stream.start_tls(self._ssl_context)
        return stream
